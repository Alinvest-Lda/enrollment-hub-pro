        ...
        <Card className="shadow-card-hover bg-card/95 backdrop-blur-xl border-border/50 rounded-2xl overflow-hidden">
          ...
          <CardContent className="p-8">
            <AnimatePresence mode="wait">
              {forgotMode ? (
                <motion.form key="forgot" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }} onSubmit={handleForgotPassword} className="space-y-5">
                  <button type="button" onClick={() => setForgotMode(false)} className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground transition-colors mb-2">
                    <ArrowLeft className="w-3.5 h-3.5" /> Voltar ao login
                  </button>
                  <div>
                    <p className="text-sm text-muted-foreground mb-4">Introduza o seu email para receber um link de recuperação da palavra-passe.</p>
                    <Label htmlFor="reset-email" className="flex items-center gap-1.5 mb-2 text-sm font-medium">
                  ...
                  <Button type="submit" variant="navy" className="w-full rounded-xl h-11 font-semibold" disabled={resetLoading}>
                    {resetLoading ? "A enviar..." : "Enviar link de recuperação"}
                  </Button>
                </motion.form>
              ) : (
                <motion.form key="login" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.2 }} onSubmit={handleLogin} className="space-y-5">
                  <div>
                  ...
                  <button type="button" onClick={() => setForgotMode(true)} className="w-full text-center text-sm text-muted-foreground hover:text-accent transition-colors">
